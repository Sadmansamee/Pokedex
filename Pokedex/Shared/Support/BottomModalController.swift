...
final class BottomModalController: UIPresentationController {
    fileprivate let popupHeight: CGFloat = UIScreen.main.bounds.size.height / 2
    ...
    @objc func didPan(pan: UIPanGestureRecognizer) {
        ...
        case .changed:
            ...
            switch state {
            case .interaction:
                presented.frame.origin.y = location.y + (UIScreen.main.bounds.size.height - popupHeight)
            case .presentation:
                presented.frame.origin.y = location.y
            }
            direction = velocity.y
        case .ended:
            let maxPresentedY = container.frame.height - (UIScreen.main.bounds.size.height - popupHeight)
    ...
    @objc func didTap(tap _: UITapGestureRecognizer) {
        presentedViewController.dismiss(animated: true, completion: nil)
    }
    ...
    override var frameOfPresentedViewInContainerView: CGRect {
        ...
        return CGRect(x: 0, y: UIScreen.main.bounds.size.height - popupHeight, width: container.bounds.width, height: popupHeight)
    }
